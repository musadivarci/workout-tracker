...
import { Minus, Plus, Save, Dumbbell, LogOut, ChevronLeft, ChevronRight, Check, Play, Flag, TrendingUp } from "lucide-react";
...
function Exercise({ex,workoutId,reload,onNeedWorkout}){
 ...
 return <article className={`card ${saved?"saved-card":""}`}><div className="head"><div><h3>{ex.name}</h3><small>Artış adımı {ex.increment_kg} kg{ex.last_increase_at?` · Son artış ${new Date(ex.last_increase_at).toLocaleDateString("tr-TR")}`:""}</small></div>{saved?<span className="done"><Check/> Kaydedildi</span>:pct>0&&<span className="badge"><TrendingUp/> +%{pct}</span>}</div>{ex.last_weight!=null&&<div className="previous">Önceki: <b>{ex.last_weight} kg</b> · {(ex.last_reps||[]).join(" / ")}</div>}{all12&&<div className="suggestion">3×12 tamamlandıysa sonraki antrenman önerisi: <b>{suggested} kg</b></div>}<label>Ağırlık</label><Stepper value={kg} step={Number(ex.increment_kg)} onChange={v=>{setKg(v);setSaved(false)}} suffix="kg"/><label>Tekrarlar</label><div className="sets">{reps.map((r,i)=><div className="set" key={i}><span>Set {i+1}</span><Stepper value={r} onChange={v=>{let x=[...reps];x[i]=v;setReps(x);setSaved(false)}}/></div>)}</div><button className={`save ${saved?"saved-btn":""}`} onClick={save} disabled={busy}>{saved?<><Check/> Güncelle</>:<><Save/> {busy?"Kaydediliyor…":"Kaydet"}</>}</button></article>
}

export default function Home(){
 const [session,setSession]=useState(null),[loading,setLoading]=useState(true),[day,setDay]=useState(1),[items,setItems]=useState([]),[workoutId,setWorkoutId]=useState(null),[startedAt,setStartedAt]=useState(null);
 ...
 useEffect(()=>{load();const key=`active-workout-${day}`;const raw=typeof window!=="undefined"?localStorage.getItem(key):null;if(raw){try{const x=JSON.parse(raw);setWorkoutId(x.id);setStartedAt(x.startedAt)}catch{setWorkoutId(null);setStartedAt(null)}}else{setWorkoutId(null);setStartedAt(null)}},[session,day]);
 async function ensureWorkout(){if(workoutId)return workoutId;const {data:{user}}=await supabase.auth.getUser();const w=await supabase.from("workouts").insert({user_id:user.id,workout_day:day}).select("id,performed_at").single();if(w.error){alert(w.error.message);return null;}setWorkoutId(w.data.id);setStartedAt(w.data.performed_at);localStorage.setItem(`active-workout-${day}`,JSON.stringify({id:w.data.id,startedAt:w.data.performed_at}));return w.data.id;}
 function finishWorkout(){localStorage.removeItem(`active-workout-${day}`);setWorkoutId(null);setStartedAt(null);load();window.scrollTo({top:0,behavior:"smooth"});}
 ...
 return <main className="shell"><header><div><small>WORKOUT TRACKER</small><h1>{d.t}</h1></div><button className="icon" onClick={()=>supabase.auth.signOut()}><LogOut/></button></header><nav><button onClick={()=>setDay(day===1?3:day-1)}><ChevronLeft/></button><div><b>Gün {day}</b><span>{d.t}</span></div><button onClick={()=>setDay(day===3?1:day+1)}><ChevronRight/></button></nav><section className={`sessionbar ${workoutId?"active":""}`}><div><span>{workoutId?"AKTİF ANTRENMAN":"BUGÜN"}</span><b>{workoutId?`Başlangıç ${new Date(startedAt).toLocaleTimeString("tr-TR",{hour:"2-digit",minute:"2-digit"})}`:"Hazır olduğunda başlat"}</b></div>{workoutId?<button onClick={finishWorkout}><Flag/> Bitir</button>:<button onClick={ensureWorkout}><Play/> Başlat</button>}</section><div className="list">{items.map(x=><Exercise key={x.id} ex={x} workoutId={workoutId} reload={load} onNeedWorkout={ensureWorkout}/>)}</div></main>
}
